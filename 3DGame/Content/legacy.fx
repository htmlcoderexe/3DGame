...
{	
	...
	Output.Fog = pow(Output.clipDistances / 125.0f,1.0f);
	Output.TW = inWeights;
	//Output.Color.rgb=outNormal.xyz;
	return Output;    
}
...
PixelToFrame TexturedTintedPS(VertexToPixel PSIn) 
{
	...
	GrassColor=tex2D(GrassSampler, PSIn.TextureCoords) * PSIn.Color;
	  GrassColor = (((tex2D(GrassSampler, PSIn.TextureCoords) * 2) - 1) * PSIn.Color + PSIn.Color );// + (((tex2D(TextureSampler, PSIn.TextureCoords * 4.0f) * 2) - 1) * PSIn.Color + PSIn.Color / 2);
	...
	//Output.Color=PSIn.Color+GrassColor*0.0f;
	return Output;
}
